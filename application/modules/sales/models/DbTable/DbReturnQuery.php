...
class sales_Model_DbTable_DbReturnQuery extends Zend_Db_Table_Abstract
{
	...
	{	
  		$db=$this->getAdapter();
		$sql = "SELECT r.return_no,r.invoice_no,r.date_return,r.remark,r.all_total,
				c.cust_name,c.add_name, c.contact_name, c.phone,l.Name
   ...
   public function getReturnInItem($return_in){//for get product items of return in
	   	$db = $this->getAdapter();
	   	$sql="SELECT r.qty_return ,r.price,r.sub_total,r.return_remark,CONCAT(p.item_name,' (',p.p_code,' )') AS item_name
   ...
   public function getReferenceInfo($return_id){//for get product items of return in
	   	$db = $this->getAdapter();
	   	$sql="SELECT ri.return_no,ro.returnout_no, ro.date_return, ro.remark,ro.all_total,l.Name
		   	  FROM tb_return_customer_out AS ro, tb_return_customer_in AS ri,tb_sublocation AS l
		   	  WHERE ri.return_id = ro.returnin_id AND l.LocationId = ro.location_id AND ro.returnout_id = ".$return_id." LIMIT 1";
	   	$rows = $db->fetchRow($sql);
   		return  $rows;
   }
   public function getReturnItemOut($id){
   		$db=$this->getAdapter();
   		$sql = "SELECT CONCAT(p.item_name,' (',p.p_code,' )') AS item_name, ri.qty_return, ri.price, ri.sub_total,ri.return_remark 
